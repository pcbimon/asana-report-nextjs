/**
 * Supabase Edge Function for daily Asana data synchronization
 * This function replaces the UI-triggered sync with automated daily sync
 */
...
serve(async (req: Request) => {
  ...
  try {
    console.log('Asana sync function started');
    
    // Get environment variables
    const asanaToken = Deno.env.get('ASANA_TOKEN');
    const asanaProjectId = Deno.env.get('ASANA_PROJECT_ID');
    const asanaTeamId = Deno.env.get('ASANA_TEAM_ID');
    const rateLimit = parseInt(Deno.env.get('RATE_LIMIT') || '150');
    
    if (!asanaToken || !asanaProjectId || !asanaTeamId) {
      throw new Error('Missing required environment variables: ASANA_TOKEN, ASANA_PROJECT_ID, ASANA_TEAM_ID');
    }
